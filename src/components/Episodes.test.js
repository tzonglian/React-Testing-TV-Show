      ...
      episodes: [
        {
          ...
          summary:
            "<p>A young boy mysteriously disEpisodesears, and his panicked mother demands that the police find him. Meanwhile, the boy's friends conduct their own search, and meet a mysterious girl in the forest.</p>",
          _links: {
            self: {
...
describe("Episodes tests", () => {
  ...
  test("renders episodes when new episode data is added", async () => {
    mockFetchShow.mockResolvedValueOnce([]);
    const { rerender } = render(<Episodes episodes={[]} />);

    // Test No Episodes
    expect(screen.queryAllByTestId("episodeData")).toStrictEqual([]);

    // Test All Episodes
    rerender(<Episodes episodes={mockShowData.data._embedded.episodes} />);
    console.log(screen.queryAllByTestId("episodeData"));
    expect(screen.queryAllByTestId("episodeData")).toHaveLength(26);
  });
  ...
  //   //Click Dropdown, Testing Season 2 Ep's Rendering
  //   const dropDown = await screen.findByText(/Select a season/i);
  //   userEvent.click(dropDown);
  //   userEvent.click(await screen.findByText(/Season 2/i));
  // });
});
